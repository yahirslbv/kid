...
import '../data/geometry_questions.dart';
import '../models/geometry_question.dart';
import '../utils/kids_question_selector.dart';
import '../widgets/geometry_shape_preview.dart';

class GeometryActivityScreen extends StatefulWidget {
  const GeometryActivityScreen({super.key});

  @override
  State<GeometryActivityScreen> createState() => _GeometryActivityScreenState();
}

class _GeometryActivityScreenState extends State<GeometryActivityScreen> {
  static const _accentColor = Color(0xFF00A6A6);
  ...
  late List<GeometryQuestion> _questions;

  GeometryQuestion get _currentQuestion => _questions[_currentIndex];
  ...
  void _startNewRound() {
    _questions = selectRandomQuestions(geometryQuestions);
    _currentIndex = 0;
    _score = 0;
...
class _GeometryQuestionView extends StatelessWidget {
  static const _accentColor = Color(0xFF00A6A6);
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                if (question.sides != null || question.vertices != null) ...[
                  const SizedBox(height: 8),
                  Text(
                    _propertyLabel(question),
                    textAlign: TextAlign.center,
                    style: TextStyle(
                      color: mutedColor,
                      fontSize: 13,
  ...
  String _propertyLabel(GeometryQuestion question) {
    final sides = question.sides;
    final vertices = question.vertices;
    if (sides == null && vertices == null) return '';
    return 'Lados: ${sides ?? '-'}  •  Vértices: ${vertices ?? '-'}';
  }
}

class _GeometryAnswerButton extends StatelessWidget {
  final String answer;
  ...
  _GeometryButtonColors _resolveColors(bool isDark) {
    if (showResult && isCorrectAnswer) {
      return const _GeometryButtonColors(
    ...
    if (isSelected) {
      return const _GeometryButtonColors(
        background: Color(0xFFE4FAFA),
        border: Color(0xFF00A6A6),
        foreground: Color(0xFF1A2D4A),
      );
    }

    return _GeometryButtonColors(
...
class _GeometryFeedback extends StatelessWidget {
  final bool isCorrect;
  final String explanation;
  final String hint;

  const _GeometryFeedback({
...
class _GeometryFinished extends StatelessWidget {
  static const _accentColor = Color(0xFF00A6A6);
...
class _GeometryButtonColors {
  final Color background;
  final Color border;
  final Color foreground;

  const _GeometryButtonColors({
